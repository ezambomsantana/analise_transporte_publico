...
def load_districts(vehicle, orde, data):
    ...
    data17_copy['DISTANCE'] = data17_copy.apply(lambda x: calculate_distance(x), axis=1)
    data17_copy['DISTANCE'] = data17_copy['DISTANCE'] / 1000
    
    data_mp2 = data17_copy[[orde,  'FE_VIA']].groupby([orde]).sum().sort_values(by=['FE_VIA']).reset_index()
    data_mp2 = data_mp2.set_index(orde)

    data_distance = data17_copy[[orde,  'DISTANCE']].groupby([orde]).mean().sort_values(by=['DISTANCE']).reset_index()
    data_distance = data_distance.set_index(orde)

    df = mapa.set_index('NomeDistri').join(data_mp2).join(distritos_pop).join(data_distance)
    df = df.reset_index()
...
for x in range(0, 4):
    ...
    fig, ax = plt.subplots(1, 1)
    teste2.plot(column='DISTANCE', ax=ax, legend=True, cmap=cmap)

    plt.axis('off')
    plt.savefig(folder_images_maps + nome + '_distancia.png', bbox_inches='tight', pad_inches=0.0)

    teste2[['NomeDistri','densidade_area', 'FE_VIA','area','Populacao','densidade_pop', 'DISTANCE']].to_csv('../generated_data/' + nome + '_transporte.csv')
...
for x in range(0, 4):
    ...
    plt.axis('off')
    plt.savefig(folder_idosos_images_maps + nome + '_distancia_idoso.png', bbox_inches='tight', pad_inches=0.0)

    teste2[['NomeDistri','densidade_area', 'FE_VIA','area','Populacao','densidade_pop','DISTANCE']].to_csv('../idoso_generated_data/' + nome + '_transporte.csv')